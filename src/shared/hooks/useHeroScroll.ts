...
export function useHeroScroll(): UseHeroScrollReturn {
  ...
  const transitionToVideoFromBlue = () => {
    ...
    setVideoEnded(false); // 비디오 상태 리셋
    console.log("[useHeroScroll/전환완료] 블루섹션 → 비디오 전환 완료");
  };

  // 비디오가 끝났을 때 호출되는 함수
  const onVideoEnd = () => {
    console.log("[useHeroScroll/비디오종료] 비디오 재생 완료");
    setVideoEnded(true);

    // 비디오가 끝나면 자동으로 블루섹션으로 전환
    setTimeout(() => {
      setShowBlueSection(true);
      setShowVideoSection(false); // 비디오 섹션 숨기기
      console.log(
        "[useHeroScroll/자동전환] 비디오 종료 후 자동으로 블루섹션 표시"
      );
    }, 1000); // 1초 후 전환
  };
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (showBlueSection) {
        ...
        if (e.deltaY < 0) {
          ...
          if (scrollTop < 5) {
            console.log(
              "[useHeroScroll/블루섹션] 최상단에서 위 스크롤 - 비디오로 복귀"
            );
            e.preventDefault();
            setShowBlueSection(false);
            setShowVideoSection(true);
        ...
        console.log(
          "[useHeroScroll/휠이벤트] 블루섹션 - 완전 자유 스크롤 허용"
        );
        return;
      }
      ...
      if (deltaY > 0) {
        ...
        if (enableNormalScroll) {
          ...
          console.log(
            "[useHeroScroll/휠다운] 파란색 섹션 → 일반 스크롤 활성화"
          );
        } else if (showVideoSection && !showBlueSection) {
          // 비디오 섹션에서 추가 스크롤 시 비디오가 끝났는지 확인
          if (videoEnded) {
            // 비디오가 끝났으면 파란색 섹션 표시
            setShowBlueSection(true);
            setShowVideoSection(false); // 비디오 섹션 숨기기
            console.log(
              "[useHeroScroll/휠다운] 비디오 종료 후 → 파란색 섹션 표시"
            );
          } else {
            // 비디오가 아직 끝나지 않았어도 스크롤로 건너뛸 수 있음
            setShowBlueSection(true);
            setShowVideoSection(false); // 비디오 섹션 숨기기
            console.log(
              "[useHeroScroll/휠다운] 비디오 재생 중이지만 스크롤로 건너뛰기 → 파란색 섹션 표시"
            );
          }
        } else if (!showVideoSection && !showBlueSection) {
  ...
  }, [showVideoSection, showBlueSection, enableNormalScroll, videoEnded]);

...
export const TABLET_BASE_WIDTH = 768; // 태블릿 기준
...
/**
 * 375px 기준으로 mvw 단위 계산 (모바일 375px~767px 구간)
...
/**
 * 768px 기준으로 tvw 단위 계산 (태블릿 768px~1023px 구간)
 * @param pixelValue - 768px 디자인에서의 픽셀 값
 * @returns vw 단위 문자열 (태블릿 기준)
 */
export const tvw = (pixelValue: number): string => {
  return `${(pixelValue / TABLET_BASE_WIDTH) * 100}vw`;
};